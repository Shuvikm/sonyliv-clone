...
const OMDB_API_KEY = 'demo'; // Using demo key for testing - replace with your actual key
const NEWS_API_KEY = 'demo'; // Using demo key for testing - replace with your actual key
const SPORTS_API_KEY = 'demo'; // Using demo key for testing - replace with your actual key
  ...
  try {
    ...
    if (response.data.Response === 'True' && response.data.Search) {
      return response.data.Search.map(movie => ({
        _id: movie.imdbID,
        title: movie.Title,
        description: `Released in ${movie.Year}`,
        type: 'movie',
        genre: [movie.Type],
        poster: movie.Poster !== 'N/A' ? movie.Poster : 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=300&h=450&fit=crop',
        rating: 0,
    ...
    // If no results from API, return filtered popular movies
    return getPopularMovies().filter(movie => 
      movie.title.toLowerCase().includes(query.toLowerCase()) ||
      movie.description.toLowerCase().includes(query.toLowerCase())
    );
  } catch (error) {
    console.error('Error searching movies:', error);
    // Return filtered popular movies as fallback
    return getPopularMovies().filter(movie => 
      movie.title.toLowerCase().includes(query.toLowerCase()) ||
      movie.description.toLowerCase().includes(query.toLowerCase())
    );
...
// Get popular movies (fallback data)
...
export const getMovieDetails = async (imdbID) => {
  try {
    ...
    if (response.data.Response === 'True') {
      return {
        _id: response.data.imdbID,
        title: response.data.Title,
        description: response.data.Plot,
        type: 'movie',
        genre: response.data.Genre ? response.data.Genre.split(', ') : [],
        poster: response.data.Poster !== 'N/A' ? response.data.Poster : 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=300&h=450&fit=crop',
        ...
        duration: response.data.Runtime ? parseInt(response.data.Runtime) : 0,
